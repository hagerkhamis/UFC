    ...
    return await _request<T>(requestMethod, url,
      ...
      if (result.code == ErrorStatus.rEQUESTDATAOK) {
        if (isList) {
          ...
          if (onSuccess != null) {
            if (result.data != null) {
              // data could legitimately be null when API returns empty payload,
              // but callers expect a concrete value.  Treat null as failure so
              // we don't crash on cast.
              onSuccess(result.data as T);
            } else {
              // invoke error handler if available, otherwise just log
              if (onError != null) {
                _onError(ErrorStatus.pARSEERROR,
                    'response data is null', onError);
              } else {
                Log.e('requestDataFuture: null data for $url');
              }
            }
          }
        }
